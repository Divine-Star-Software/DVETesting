...
export function RegisterCommands() {
  ...
  commands.registerCommand({
    ...
    arguments: false,
    run(onDone, io, args) {
      const p1: string[] = args.input.trim().split(" ");
      if (p1.length < 3) {
      ...
      DVER.nexusComm.runTasks("set-player-position", [
        position[0],
        position[1],
        position[2],
      ]);
  ...
  commands.registerCommand({
    ...
    arguments: false,
    run(onDone, io, args) {
      const value: number = Number(args.input.trim());
      if (value) {
  ...
  commands.registerCommand({
    ...
    run(onDone, io, args) {
      ...
      const listener = (event: KeyboardEvent) => {
        if (event.key != "Escape" && event.key != "F3") return;
        clearInterval(inte);
