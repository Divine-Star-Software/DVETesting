...
export const PlayerAudio = {
    INIT(player) {
        ...
        DVER.TC.registerTasks("set-material", (data) => {
        ...
        player.nodes.scene.registerBeforeRender(() => {
            DAE.space.setListenerPosition(player.manager.physics.position.x, player.manager.physics.position.y, player.manager.physics.position.z);
            DAE.space.setListenerDirection(player.manager.physics.direction.x * -1, player.manager.physics.direction.y, player.manager.physics.direction.z * -1);
        });
        setInterval(() => {
            const maxDelta = player.manager.physics.is.running ? 400 : 800;
            if (player.manager.physics.is.walking &&
