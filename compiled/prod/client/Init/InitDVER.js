...
export async function $INIT_DVER() {
 ...
 `,
    ...
    const player = await INIT_RENDER_PLAYER(scene, camera, DVER, {
        CreateBox,
        EdgesRenderer,
        StandardMaterial,
    }, model);
...
const $INITWorkers = () => {
    const worldWorker = new Worker(new URL("../Contexts/World/world.js", import.meta.url), {
        type: "module",
    });
    const constructorWorkers = [];
    for (let i = 0; i < navigator.hardwareConcurrency - 2; i++) {
        constructorWorkers.push(new Worker(new URL("../Contexts/Constructor/Constructor.js", import.meta.url), {
            type: "module",
        }));
    }
    const nexusWorker = new Worker(new URL("../Contexts/Nexus/nexus.js", import.meta.url), {
        type: "module",
    });
    const dataWorker = new Worker(new URL("../../environments/electron/client/dataloader.js", import.meta.url), {
        type: "module",
    });
    /*
   ...
   */
    const richWorldWorker = new Worker(new URL("../Contexts/RichWorld/richworld.js", import.meta.url), {
        type: "module",
    });
    return {
