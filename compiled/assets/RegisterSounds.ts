...
export async function RegisterSounds() {
  const voxelPathHiFi = (name: string, extension = "mp3") => {
    return `./assets/audio/sfx/hifi/${name}.${extension}`;
  };
  const voxelPathLoFi = (name: string, extension = "mp3") => {
    return `./assets/audio/sfx/lofi/${name}.${extension}`;
  };

  const walkPathHiFi = (name: string, extension = "mp3") => {
    return `./assets/audio/sfx/hifi/walking/${name}.${extension}`;
  };
  const walkPathLoFi = (name: string, extension = "mp3") => {
